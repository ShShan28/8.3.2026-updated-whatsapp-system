...
async function executeScheduledJob(job, instanceId, token) {
    ...
    for (let i = 0; i < totalRecipients; i++) {
        ...
        try {
            ...
            let body = parseMessageVariables(job.message, name, scheduleFilename, job.date);
...
/**
 * --------------------------------------------------------------------------
 * UPGRADED VARIABLE ENGINE (Name, Date, Day, Filename)
 * --------------------------------------------------------------------------
 */

/**
 * Returns the Tamil Day Name for a given date object
 */
function getTamilDay(dateObj = new Date()) {
    const daysTamil = [
    ...
    return daysTamil[dateObj.getDay()];
}

/**
 * Generates date in DD-TamilMonth-YYYY (Gregorian style)
 * Accepts an optional Date object for scheduling support
 */
function getTamilDate(dateObj = new Date()) {
    ...
    const month = gregorianMonthsTamil[dateObj.getMonth()]; 
...
/**
 * UNIVERSAL VARIABLE PARSER
 * Handles: {name}, {date}, {day}, {filename}
 * Use: targetDateStr is passed only by the Scheduler to reflect future dates
 */
function parseMessageVariables(msg, name, filename = "", targetDateStr = null) {
    if (!msg) return " ";
    
    // Determine the source date (Future scheduled date OR Present)
    const sourceDate = targetDateStr ? new Date(targetDateStr) : new Date();
    
    const tamilDate = getTamilDate(sourceDate);
    const tamilDay = getTamilDay(sourceDate);
    ...
    return msg
        ...
        .replace(/{day}/g, tamilDay)
        .replace(/{filename}/g, cleanFilename);
}

/**
 * Simple formatter for string-based dates (Used for UI displays)
 */
function formatTamilDateFromStr(dateStr) {
    if (!dateStr) return "";
    return getTamilDate(new Date(dateStr));
}
/* ========================================================================== */
